// scripts/typesense-indexer-id-fix.js
...
class TypesenseIndexerIdFix {
  constructor () {
    ...
    this.batchSize = parseInt(options.batchSize) || 299
    ...
    this.logFile = `logs/typesense-id-fix-${this.timestamp}.log`
  }

  async init () {
    console.log('🔧 Iniciando Typesense Indexer ID FIX...')
    console.log('🎯 SOLUCIÓN: Usar campo "id" como clave primaria real')
    console.log(`📋 Configuración: páginas=${this.maxPages}, desde=${this.startPage}, batch=${this.batchSize}`)

    this.stats.startTime = performance.now()

    await this.ensureLogDirectory()
    await this.writeLog('INFO', '🔧 ID FIX Indexer iniciado - usando campo id')
  ...
  async validateSchema () {
    try {
      ...
      console.log('⚠️ IMPORTANTE: Usando el esquema existente pero con campo "id" como clave primaria')
    ...
    for (let attempt = 1; attempt <= API_CONFIG.maxRetries; attempt++) {
      try {
        ...
        const response = await axios.get(url, {
          ...
          headers: {
            'User-Agent': 'TypesenseIdFixIndexer/1.0',
  ...
  transformProductForTypesense (product) {
    try {
      ...
      // 🔧 CLAVE DEL FIX: Usar "id" como campo primario en lugar de "objectID"
      const document = {
        // 🎯 CAMPO CLAVE PRIMARIA REAL DE TYPESENSE
        id: String(product.id || product.external_id), // ← ESTE ES EL CAMBIO PRINCIPAL

        // Mantener objectID para compatibilidad con tu esquema
        ...
        // Todos los demás campos igual que antes
  ...
  async indexProductBatch (products) {
    ...
    try {
      ...
      await this.writeLog('INFO', `Indexando batch de ${documents.length} documentos con campo ID como clave primaria`)

      // 🔧 USAR UPSERT - ahora debería funcionar con el campo "id"
  ...
  async indexAllProducts () {
    ...
    console.log('📊 Iniciando indexación con ID FIELD FIX...')
    await this.writeLog('INFO', 'Iniciando indexación con campo id como clave primaria')

    while (hasMorePages && pagesProcessed < this.maxPages) {
      try {
  ...
  async generateReport () {
    ...
    const report = {
      summary: {
        ...
        fix: 'ID_FIELD_PRIMARY_KEY'
      },
      errors: this.stats.errors.slice(-50),
      timestamp: new Date().toISOString()
    }

    const reportFile = `logs/typesense-id-fix-report-${this.timestamp}.json`
    await fs.writeFile(reportFile, JSON.stringify(report, null, 2))

    await this.writeLog('INFO', 'RESUMEN FINAL', report.summary)

    console.log('\n📊 RESUMEN DE INDEXACIÓN TYPESENSE (ID FIELD FIX):')
    ...
    console.log('\n🎯 PARA PROBAR UPSERT:')
    console.log('node --env-file=.env scripts/typesense-indexer-id-fix.js --pages 1')
    console.log('(Ejecutar 2 veces para verificar que no duplica)')
...
// Función principal
async function main () {
  const indexer = new TypesenseIndexerIdFix()
