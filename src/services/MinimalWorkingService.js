// services/MinimalWorkingService.js
import { performance } from 'perf_hooks'

class MinimalWorkingService {
  constructor (connection) {
    this.connection = connection
    this.batchSize = 20 // Muy pequeño para máxima compatibilidad
  }
  ...
  async bulkUpsertProducts (products) {
    ...
    console.log(`📦 Procesando ${products.length} productos con método que SÍ FUNCIONA...`)
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      try {
        const result = await this.processChunkMinimal(chunk)
  ...
  async processChunkMinimal (products) {
    let inserted = 0
    let updated = 0
    let errors = 0

    // Procesar cada producto individualmente (método que SÍ funcionó)
    for (const product of products) {
      try {
        // Verificar si existe (método exacto del test que funcionó)
        ...
        if (existing.length > 0) {
          // UPDATE simple (método exacto del test que funcionó)
          ...
          // INSERT simple (método exacto del test que funcionó)
          await this.connection.execute(`
  ...
  // Métodos auxiliares copiados exactamente del test que funcionó
...
export default MinimalWorkingService
